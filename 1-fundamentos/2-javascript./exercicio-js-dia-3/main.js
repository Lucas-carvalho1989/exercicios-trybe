...
function handelChangeTech (event){
    const techElement = document.querySelector('.tech');
    techElement.classList.remove('tech');
    event.target.classList.add('tech');
    input.value = '';
}
firstLi.addEventListener('click', handelChangeTech);
secondLi.addEventListener('click', handelChangeTech);
thirdLi.addEventListener('click', handelChangeTech);
...
myWebpage.addEventListener('mouseout', (event) => {
    event.target.stule.color = 'unset';
});
